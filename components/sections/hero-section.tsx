  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <motion.div
          className="grid grid-cols-2 gap-3 pt-1 sm:gap-4"
...
function HeroActionButton({
  ...
  const baseClass =
    "focus-ring flex w-full min-h-[5.5rem] flex-col cursor-pointer items-center justify-center gap-2.5 rounded-lg border px-4 py-4 text-center text-sm font-medium leading-tight transition-colors sm:min-h-24 sm:text-base";
  ...
  const label = <span className="max-w-full text-balance break-words">{action.label}</span>;
  ...
  const icon = action.icon === "github" ? (
    ...
    <svg viewBox="0 0 24 24" className="h-6 w-6 shrink-0 fill-none stroke-current" aria-hidden>
